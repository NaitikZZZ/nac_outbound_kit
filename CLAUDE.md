...
2. **HubSpot is read-only.** Never write, update, or create anything in HubSpot. Read context only. **Two scoped exceptions:**
   ...
   - When a campaign build is linked to a HubSpot Project, Partner, and/or Event record (via the naming convention's PROJECTID lookup, or the ABM Wrapper's association picker), the pipeline may post **one** summary note to **each** associated record (Project, Partner, and Event are independent - if two or three are linked, each gets its own copy of the note) after the campaign is created, containing: campaign name, product, team, priority, lead count, segments, dream-account summary (and any other per-file processing summary shared with the user), Smartlead campaign ID (PAUSED), HeyReach list ID (if any), and GTM-enrichment Claude token/cost usage. Confirmed by the user on 2026-09-16; expanded from Project-only to also cover Partner and Event on 2026-09-18. The live ABM Wrapper (`wrapper/`) does this autonomously per run - see `wrapper/backend/app/pipeline/hubspot_project_note.py` in `NaitikZZZ/smartlead-kit`, which loops over whichever of the three association kinds (`project` / `partner` / `event`) are actually present on the run and posts to each (object type IDs `0-970` / `2-17592276` / `2-229555311`).
