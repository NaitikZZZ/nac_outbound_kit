...
`references/edge-cases.md` lists every case the script handles, what it does, and
the ones it deliberately punts to a flag. Read it before extending the script or
hand-fixing a batch.

`references/prompt.md` is a standalone prompt for normalizing a small pasted list
...
there rather than writing new regexes. After any edit, re-run against a real
export from your own CRM or list-building tool and diff the flag histogram —
lookup-table edits are easy to get subtly wrong, and a live file catches that
faster than reasoning about the regex.
