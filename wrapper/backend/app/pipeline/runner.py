...
If not specified, use previous filters. Return ONLY JSON, no markdown."""
        ...
        def _ask_step(base_key, *a, **kw):
            # Every ask() needs a key the frontend hasn't seen before - StepCard
            # is keyed by question.key and only remounts (clearing its
            # "answered" flag) on a genuinely new key, so redoing the same
            # kind after "back" or a not-found retry needs a fresh suffix too.
            nonlocal _ask_n
            _ask_n += 1
            return ask(run_id, f"{base_key}_{_ask_n}", *a, **kw)

        def _refresh_cache_async(kind: str):
            # A record resolved outside the cached dropdown means it's new
            # since the last cron run - self-heal the cache now instead of
            # making the team wait for the next scheduled refresh. Runs in the
            # background (tens of seconds) so it doesn't block the pipeline.
            def _run():
                try:
        ...
        def _resolve_kind(kind: str, allow_back: bool):
            ...
            # Not in the cached dropdown (fetch failed, empty cache, or the
            # user chose "Other") - resolve live against HubSpot instead of
            ...
            while True:
                ...
                if resolved["status"] == "not_found":
                    ...
                    _set_message(run_id, f"No {kind} found matching {value!r} - try again.")
                    continue
                return ("ok", resolved["record_id"], True)

        while True:
            assoc_kinds_answer = _ask_step(
            ...
            i = 0
            while i < len(kinds):
                outcome = _resolve_kind(kinds[i], allow_back=(i > 0))
